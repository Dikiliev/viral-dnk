...
const LandingPage: React.FC<{ onStart: () => void }> = ({ onStart }) => {
  return (
    <div className="flex flex-col items-center py-16 relative min-h-screen w-full">
      ...
      {/* Main Content */}
      <div className="text-center space-y-8 max-w-4xl px-4 animate-in fade-in slide-in-from-bottom-4 duration-700">
        {/* Badge */}
        <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full border border-purple-500/20 bg-purple-500/10 text-purple-600 dark:text-purple-400 text-[11px] font-bold uppercase tracking-[0.2em] mb-4 shadow-sm hover:scale-105 transition-transform">
        ...
        <h1 className="text-5xl md:text-7xl font-[800] tracking-tight leading-[1.1] text-slate-900 dark:text-white">
          Раскрой <br />
          <span className="bg-clip-text text-transparent bg-gradient-to-r from-brand-600 via-pink-500 to-orange-500 animate-gradient">
            ДНК Виральности
          </span>
        </h1>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-6 pt-6">
          <button 
            onClick={onStart}
            className="group relative bg-brand-600 text-white px-10 py-5 rounded-[20px] font-bold text-xl hover:scale-105 active:scale-95 transition-all shadow-xl shadow-brand-600/25 hover:shadow-brand-600/40 overflow-hidden"
          >
          ...
          <div className="flex items-center gap-3">
            <div className="flex -space-x-2">
              {[1,2,3,4].map(i => (
                <div 
                  key={i} 
                  className="w-10 h-10 rounded-full border-2 border-white dark:border-slate-900 bg-slate-200 dark:bg-slate-800 flex items-center justify-center text-[10px] font-bold text-slate-500 shadow-sm hover:scale-110 transition-transform"
                >
      ...
      {/* Features Grid */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 w-full max-w-6xl mt-32 px-4">
        {[
          {
          ...
          <div 
            key={i} 
            className="glass p-10 rounded-[40px] text-left hover:translate-y-[-4px] transition-all duration-300 group relative overflow-hidden cursor-pointer"
          >
            <div className={`absolute top-0 right-0 w-32 h-32 bg-gradient-to-br ${feature.color} to-transparent blur-2xl opacity-50 group-hover:opacity-100 transition-opacity`}></div>
